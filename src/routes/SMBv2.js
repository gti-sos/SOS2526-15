...
const API_URL_SMB = "/api/v2/minimum-interprofessional-wages";
let db = new dataStore();

export function loadBackendSMBv2(app){
  ...
  app.get(`${API_URL_SMB}/loadInitialData`, (req, res) => {
   ...
   app.get(API_URL_SMB, (req, res) => {
    ...
    query.date = {};
        ...
        if (req.query.from) {
            query.date.$gte = parseInt(req.query.from);
        }

        if (req.query.to) {
            query.date.$lte = parseInt(req.query.to);
        }
    }
