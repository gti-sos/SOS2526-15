<script>
    ...
    async function loadHealthyDiet() {
        clearContainers();
        const res = await fetch("https://sos2526-18.onrender.com/api/v1/cost-of-healthy-diet-by-countries");
        if(res.ok){
            ...
            setTimeout(() => {
                Highcharts.chart('chartHealthyDiet', {
                    chart: { type: 'area' },
                    title: { text: 'Healthy Diet Cost' },
                    xAxis: { categories: healthyDietData.slice(0,10).map(d => d.country) },
                    series: [{ name: 'Cost (USD)', data: healthyDietData.slice(0,10).map(d => d.cost_healthy_diet_ppp_usd) }]
                });
            },100);
        }
    }
...
<main class="container py-4">
    ...
    <div class="content-area p-4 border rounded bg-white shadow-sm" style="min-height: 500px;">
        ...
        {:else if currentIntegration === "SMB_G18"}
            <h3>🥗 Healthy Diet</h3>
            <div id="chartHealthyDiet" style="width:100%; height:400px;"></div>
        {/if}
    </div>
</main>
